...
$query = DBRead('ecommerce_lista_categoria','*', "WHERE `id_lista` = $id");
?>
...
<div class="card">
        ...
        <div>
          <table id="DataTable" class="table m-0 table-striped">
            ...
            <?php foreach ($query as $dados) {
              $query = DBRead('ecommerce_categorias','*', "WHERE id = {$dados['id_categoria']}");
              $categoria = $query[0];
              ?>
              <tr>
  ...
  <?php } else { ?>
    <div class="card-body">
    <?php if (checkPermission($PERMISSION, $_SERVER['SCRIPT_NAME'], 'listagem', 'adicionar')) { ?>
      <div class="alert alert-info">Nenhum categoria a essa listagem adicionada até o momento, <a class="adicionarListagemItem" href="#" data-href="ecommerce.php?AdicionarItemLista=<?php echo $id ; ?>">clique aqui</a> para adicionar.</div>
      <?php } ?>
    </div>
  <?php } ?>
</div>
